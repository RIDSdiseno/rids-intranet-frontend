...
type EquipoAgentFull = EquipoRow & {
  hostname?: string | null;
  usuarioActual?: string | null;
  dominio?: string | null;
  localIp?: string | null;
...
const EquiposPage: React.FC = () => {
  ...
  async function fetchEquipoAgentDetalle(id_equipo: number, signal?: AbortSignal) {
    setAgentLoading(true);
    setAgentError(null);

    try {
      const res = await http.get(`/equipos/agent/${id_equipo}`, { signal });
      setViewAgent(res.data.equipo ?? null);
    } catch (err) {
      if ((err as Error).name === "AbortError") return;
      setAgentError((err as Error).message || "Error al cargar datos del agente");
      setViewAgent(null);
    } finally {
  ...
  function agenteEstadoClasses(estado?: string | null) {
    switch (estado) {
      case "ACTIVO":
  ...
  function diskUsedPercent(equipo?: EquipoAgentFull | null) {
    if (!equipo?.diskTotalGb || equipo.diskFreeGb === null || equipo.diskFreeGb === undefined) {
      return null;
    }
  ...
  const startView = (row: EquipoRow) => {
    ...
    fetchEquipoAgentDetalle(row.id_equipo, c.signal);
  ...
  const loadSolicitantesEdit = async (empresaId: number | null, term: string) => {
    ...
    setMarcaFilter((prev) =>
      prev.toLowerCase() === b.toLowerCase() ? "" : b
    );
          ...
          {!loading &&
            ...
            rowsSorted.map((e) => (
              ...
              >
                ...
                {(e as any).agenteActivo ? (
                  <span
                    className={clsx(
                      "inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-medium border",
                      agenteEstadoClasses((e as any).estadoAgente)
                    )}
                  >
                    Agente {(e as any).estadoAgente || "ACTIVO"}
                  </span>
                ) : null}
                    ...
                    <div className="mt-1 space-y-1">
                      {splitAgentText(
                        ...
                        splitAgentText(
                          ...
                          <div
                            key={`office-manual-${idx}`}
                            className="rounded-lg bg-slate-50 px-2 py-1 text-slate-700"
                          >
                            {line}
                          </div>
                        ))
                ...
                <div className="rounded-2xl border border-cyan-200 bg-white p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <MultiInfoLine
                      ...
                      value={viewAgent.detalle?.office || viewAgent.office || viewRow?.office}
                    />
