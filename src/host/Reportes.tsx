...
const ReportesPage: React.FC = () => {
  ...
  const fetchMantencionesRemotas = async (
    ...
    const first = new URL(`${API_URL}/mantenciones-remotas`);
    first.searchParams.set("page", "1");
    first.searchParams.set("pageSize", "500");
    first.searchParams.set("empresaId", empresaId);
    ...
    const all: MantencionRemotaRow[] = [...(j1.items ?? j1.data ?? [])];
  ...
  const obtenerDatosReporteGeneral = async (): Promise<ReporteGeneralData> => {
    ...
    const [solicitantes, equipos, visitas, tickets, mantencionesRemotas] = await Promise.all([
    ...
    const mantencionesFiltradas = mantencionesRemotas.filter(
      (mr: MantencionRemotaRow) =>
    ...
    return {
      ...
      empresaFiltro,
      mantencionesRemotas: mantencionesFiltradas
    };
  };
  ...
  const previsualizarDatos = async () => {
    try {
      ...
      await generateBarChart(
        "chart-preview-mantenimientos",
        contarMantenimientos(data.visitas).map(r => r.Ítem),
        contarMantenimientos(data.visitas).map(r => r.Cantidad),
  ...
  const exportDOCX = async () => {
    try {
      ...
      const docChildren: (Paragraph | Table)[] = [
        ...
        Body(
          `Durante el periodo ${periodoTexto} se ejecutaron ${data.mantencionesRemotas?.length ?? 0
          } mantenciones remotas registradas en sistema.`
        ),

        ...tablePro(
          "Detalle de mantenciones remotas.",
          ["ID", "Técnico", "Inicio", "Fin", "Estado", "Usuario"],
          (data.mantencionesRemotas ?? []).map((m) => ({
            ID: m.id_mantencion,
            Técnico: m.tecnico?.nombre ?? "—",
            Inicio: m.inicio
              ? new Date(m.inicio).toLocaleString("es-CL")
              : "—",
            Fin: m.fin
              ? new Date(m.fin).toLocaleString("es-CL")
              : "—",
      ...
      <main className="flex-1 p-6 max-w-7xl mx-auto w-full">
        ...
        >
          ...
          <div style={{ fontSize: 14, marginBottom: 12 }}>
            Durante el periodo se ejecutaron{" "}
            <strong>
              {(dataPrev?.mantencionesRemotas || []).length}
            </strong>
            actividades de mantención remota.
          </div>
            ...
            <tbody>
              {(dataPrev?.mantencionesRemotas || []).map((m) => (
                <tr key={m.id_mantencion}>
                  ...
                  <td style={cellBorder}>
                    {m.inicio ? new Date(m.inicio).toLocaleString("es-CL") : "—"}
                  </td>
                  <td style={cellBorder}>
                    {m.fin ? new Date(m.fin).toLocaleString("es-CL") : "—"}
                  </td>
