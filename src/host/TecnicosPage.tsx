...
//
//  El problema principal: el div raíz usaba:
//    bg-gradient-to-b from-white via-white to-cyan-50
//  Este gradiente no era capturado por las reglas CSS de modo oscuro.
//
//  CAMBIOS APLICADOS:
//  1. Div raíz: reemplaza gradiente blanco por variable CSS
//  2. Header de página: bg-white/80 → bg-white dark compatible
//  3. Tabs: sin cambio de clase, cubiertos por CSS nuevo
//  4. Tabla header: from-cyan-50 to-indigo-50 → clases explícitas oscuras
//  5. Modal: sticky header bg-white → compatible
//
//  NOTA: Se mantiene el mismo comportamiento visual en modo claro.
//  Los cambios solo añaden compatibilidad dark mode sin romper lo existente.
// ────────────────────────────────────────────────────────────────────────────
...
function getMonthRangeFromKey(monthKey: string) {
  ...
  if (!Number.isFinite(year) || !Number.isFinite(month)) return getMonthRangeFromKey(getCurrentMonthKey());
...
const Modal: React.FC<{ title: string; onClose: () => void; children: React.ReactNode }> = ({ title, onClose, children }) => (
  <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-0 sm:p-4">
    {/*
      CAMBIO: bg-white → sin clase de fondo explícita + estilo CSS variable
      El CSS dark mode cubre .rounded-t-2xl y .rounded-2xl con border-cyan-200
    */}
    <div className="w-full sm:max-w-md max-h-[92dvh] overflow-y-auto rounded-t-2xl sm:rounded-2xl border border-cyan-200 bg-white shadow-xl">
      {/* sticky header — cubierto por .sticky.top-0.bg-white en el CSS */}
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white px-5 py-4">
        <h2 className="text-base font-bold text-slate-900">{title}</h2>
        <button
          onClick={onClose}
          className="rounded-lg p-1 hover:bg-slate-100 transition-colors text-slate-400"
          aria-label="Cerrar"
        >
          <CloseOutlined />
        </button>
...
const TecnicosPage: React.FC = () => {
    ...
    () =>
      tecnicos.filter((t) => {
        ...
        return (
          ...
          (filtroRol ? rol === filtroRol.toUpperCase().trim() : true)
        );
      }),
  ...
  const fetchTecnicos = useCallback(async () => {
    try {
      ...
      const res = await http.get("/tecnicos/usuarios", { params: { status: filtroStatus } });
      setTecnicos(Array.isArray(res.data) ? res.data : []);
    } catch (err: any) { setError(err?.message || "Error al cargar técnicos"); }
    finally { setLoading(false); }
  }, [filtroStatus]);

  const fetchHorasDashboard = useCallback(async () => {
    try {
      setHorasLoading(true); setHorasError(null);
      const { data } = await http.get("/tecnicos/dashboard/horas-hombre", { params: { from: horasRange.from, to: horasRange.to } });
      setHorasData(data?.data ?? null);
    } catch (err: any) {
      setHorasError(err?.response?.data?.message || err?.message || "Error al cargar dashboard");
    } finally { setHorasLoading(false); }
  }, [horasRange.from, horasRange.to]);

  useEffect(() => { fetchTecnicos(); }, [fetchTecnicos]);
  useEffect(() => { if (activeTab !== "horas") return; fetchHorasDashboard(); }, [activeTab, fetchHorasDashboard]);
  ...
  const pageNumbers = useMemo(() => {
    ...
    return nums.reduce<(number | "...")[]>((acc, n, i, arr) => {
  ...
  return (
    /*
      CAMBIO CLAVE: reemplaza el gradiente blanco:
        ANTES: bg-gradient-to-b from-white via-white to-cyan-50
        AHORA: bg-[var(--color-bg)]
      Usa la variable CSS que cambia automáticamente entre temas.
      En modo claro --color-bg = #f8fafc (prácticamente igual al blanco anterior).
    */
    <div
      ...
      <div className="mx-auto max-w-screen-2xl px-3 pt-14 pb-10 sm:px-5 sm:pt-6 lg:px-8">
            ...
            <div className="border-b border-cyan-200 bg-cyan-50 px-4 py-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-xl border border-cyan-200 bg-white px-3 py-2 text-sm text-cyan-800 hover:bg-cyan-50 transition sm:py-1.5"
                >
                ...
                {isAdminLike && (
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-600 px-3 py-2 text-sm font-semibold text-white hover:bg-cyan-700 transition sm:py-1.5"
                  >
                    + Nuevo Técnico
                  </button>
                )}
            ...
            <div className="flex flex-col gap-2 px-4 py-3 sm:flex-row sm:flex-wrap sm:items-center border-b border-slate-100">
              <div className="relative flex-1 min-w-0 sm:min-w-[220px]">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 pl-8 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400"
                />
              </div>
              <select
                value={filtroRol}
              ...
              <select
                value={filtroStatus}
                onChange={(e) => { setFiltroStatus(e.target.value as "activo" | "inactivo" | "todos"); setPage(1); }}
                className="rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 sm:w-auto w-full"
              >
              ...
              {(busqueda || filtroRol || filtroStatus !== "activo") && (
                <button
                  onClick={() => { setBusqueda(""); setFiltroRol(""); setFiltroStatus("activo"); setPage(1); }}
                  className="rounded-xl border border-slate-200 px-3 py-2 text-sm hover:bg-slate-50 transition sm:w-auto w-full"
                >
                  Limpiar filtros
                </button>
              )}
                ...
                <div className="block sm:hidden divide-y divide-slate-100">
                  {tecnicosPaginados.length === 0 ? (
                    <div className="px-4 py-10 text-center">
                      <UserOutlined className="text-3xl text-slate-300 mb-2 block" />
                      <p className="text-sm text-slate-400">{tecnicos.length === 0 ? "Sin técnicos." : "Sin coincidencias."}</p>
                    </div>
                  ) : tecnicosPaginados.map((t) => (
                    <div key={t.id_tecnico} className="px-4 py-4">
                      <div className="flex items-start gap-3">
                        <div className="w-9 h-9 rounded-full bg-cyan-50 flex items-center justify-center text-cyan-700 shrink-0">
                          <UserOutlined />
                        </div>
                  ...
                  <table className="w-full min-w-[680px] text-sm">
                    <thead className="border-b border-cyan-200 text-xs uppercase text-slate-500"
                      /*
                        ...
                        que no se cubría bien en modo oscuro, por una clase
                        bg-slate-50 que sí está en las reglas dark mode.
                      */
                    ...
                    <tbody>
                      {tecnicosPaginados.length === 0 ? (
                        ...
                        <tr
                          key={t.id_tecnico}
                          className="border-t border-slate-100 odd:bg-white even:bg-slate-50/40 hover:bg-cyan-50/60 transition-colors"
                        >
                          <td className="px-4 py-3 text-slate-400 text-xs">{t.id_tecnico}</td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <div className="w-7 h-7 rounded-full bg-cyan-50 flex items-center justify-center text-cyan-600 shrink-0 text-xs">
                                <UserOutlined />
                              </div>
            ...
            <div className="flex flex-col gap-3 border-t border-slate-100 bg-slate-50/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                <button
                  ...
                  disabled={page === 1}
                  className="rounded-xl border border-cyan-200 bg-white px-3 py-1.5 text-xs text-cyan-800 hover:bg-cyan-50 transition disabled:opacity-40"
                >
                  ← Anterior
                </button>
                {pageNumbers.map((n, i) =>
        ...
        {activeTab === "horas" && (
          <div className="space-y-5">
            <div className="rounded-2xl border border-cyan-200 bg-white px-4 py-4 shadow-sm">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-slate-500">Visitas completadas + mantenciones remotas.</p>
                </div>
                <div>
            ...
            <HorasHombreDashboard data={horasData} loading={horasLoading} error={horasError} />
          </div>
        )}
      </div>
      ...
      {creando && (
        <Modal title="Nuevo técnico" onClose={() => setCreando(false)}>
          ...
          <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            <button onClick={onCrearTecnico} disabled={guardandoNuevo} className="w-full sm:w-auto rounded-xl bg-cyan-600 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-700 transition disabled:opacity-60">
              {guardandoNuevo ? "Creando…" : "Crear técnico"}
            </button>
          </div>
        </Modal>
      )}
      ...
      {editando && (
        <Modal title="Editar técnico" onClose={() => setEditando(null)}>
            ...
            <div className="flex gap-4">
              {[true, false].map((val) => (
                <label key={String(val)} className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="formStatus" checked={formStatus === val} onChange={() => setFormStatus(val)} />
                  <span className="text-sm text-slate-700">{val ? "Activo" : "Inactivo"}</span>
                </label>
              ))}
            </div>
          </div>
          <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            <button onClick={onGuardar} disabled={guardando} className="w-full sm:w-auto rounded-xl bg-cyan-600 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-700 transition disabled:opacity-60">
