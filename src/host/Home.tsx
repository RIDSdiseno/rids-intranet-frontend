...
const toErrorMessage = (e: unknown, fallback = "Error desconocido") => {
  const ax = e as AxiosError<{ error?: string; message?: string } | string>;

  if (ax?.response?.data) {
    if (typeof ax.response.data === "string") {
      return ax.response.data || fallback;
    }

    if (typeof ax.response.data === "object") {
...
type WorstTecnicoTickets = TecnicoMetric;
...
const Home: FC = () => {
  const [visibleSections, setVisibleSections] =
    useState<HomeSectionsVisibility>(() => {
      const saved = localStorage.getItem("homeVisibleSections");

      if (saved) {
        try {
          return {
            ...DEFAULT_HOME_SECTIONS,
            ...(JSON.parse(saved) as Partial<HomeSectionsVisibility>),
          };
        } catch {
          localStorage.removeItem("homeVisibleSections");
        }
      }
  ...
  const toggleSection = (section: HomeSectionKey) => {
    setVisibleSections((prev) => {
      ...
      localStorage.setItem("homeVisibleSections", JSON.stringify(next));
  ...
  const showAllSections = () => {
    setVisibleSections(DEFAULT_HOME_SECTIONS);
  ...
  const [errorTecnicosMetrics, setErrorTecnicosMetrics] = useState<string | null>(
    null
  );

  const [worstTecnicosTickets, setWorstTecnicosTickets] = useState<
    WorstTecnicoTickets[]
  >([]);
  const [loadingWorstTecnicosTickets, setLoadingWorstTecnicosTickets] =
    useState(false);
  const [errorWorstTecnicosTickets, setErrorWorstTecnicosTickets] = useState<
    string | null
  >(null);
  ...
  const fetchTecnicosMetrics = async (signal?: AbortSignal) => {
    try {
      ...
      setErrorTecnicosMetrics(
        toErrorMessage(e, "Error al cargar métricas de técnicos")
      );
      setTecnicosMetrics([]);
    } finally {
  ...
  const fetchWorstTecnicosTickets = async (signal?: AbortSignal) => {
    try {
      setLoadingWorstTecnicosTickets(true);
      setErrorWorstTecnicosTickets(null);

      const { data } = await api.get("/helpdesk/tickets/tecnicos/worst-closed", {
        params: {
          from,
          to,
          limit: 5,
      ...
      if (data?.ok) {
        setWorstTecnicosTickets(data.data ?? []);
      } else {
        setWorstTecnicosTickets([]);
      }
    } catch (e) {
      if (
        (e as Error).name === "CanceledError" ||
      ...
      setErrorWorstTecnicosTickets(
        toErrorMessage(e, "Error al cargar técnicos con menos tickets cerrados")
      );
      setWorstTecnicosTickets([]);
    } finally {
      setLoadingWorstTecnicosTickets(false);
    }
  };
  ...
  useEffect(() => {
    ...
    fetchWorstTecnicosTickets(c6.signal);
  ...
  const computedStats: Stat[] = useMemo(
    () => [
      ...
      },
      {
        ...
        change: topTecnico
          ? `${formatNumber(topTecnico.cantidad)} visitas`
          : "Sin datos",
        onRefresh: () => fetchVisitasMetrics(),
        isLoading: loadingVis,
      },
  ...
  return (
    <div className="mx-auto w-full max-w-screen-2xl px-3 py-4 sm:px-6 lg:px-8">
      <div className="space-y-5 sm:space-y-6">
        ...
        <motion.div
          className="bg-white rounded-xl shadow-md p-3 sm:p-4 border border-slate-100"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-base sm:text-lg font-bold text-slate-800">
                Personalizar estadísticas del Home
              </h2>

              <p className="text-xs text-slate-500">
                Oculta o muestra las secciones visibles en este dashboard.
              </p>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {(Object.keys(HOME_SECTIONS_LABELS) as HomeSectionKey[]).map(
              (section) => {
                const active = visibleSections[section];
        ...
        {visibleSections.visitasTecnico && (
          <motion.div
        ...
        {visibleSections.topTecnicosTickets && (
          <motion.div
            className="mt-5 sm:mt-6 bg-white rounded-xl shadow-md p-3 sm:p-4 border border-slate-100"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              >
                {loadingTecnicosMetrics ? <LoadingOutlined /> : <ReloadOutlined />}{" "}
                Refrescar
              </button>
            </div>
            ...
            {loadingTecnicosMetrics ? (
              ...
              <div className="py-6 text-neutral-400">
                No hay métricas de técnicos disponibles.
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <div className="rounded-full bg-cyan-100 text-cyan-800 text-xs font-semibold px-3 py-1">
                        {formatNumber(t.closedTickets)} cerrados
                      </div>
                    </div>
        ...
        {visibleSections.worstTecnicosTickets && (
          <motion.div
            className="mt-5 sm:mt-6 bg-white rounded-xl shadow-md p-3 sm:p-4 border border-slate-100"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-base sm:text-lg font-bold text-slate-800">
                  Top 5 peores técnicos 👎😴
                </h2>

                <p className="text-xs text-slate-500">
                  Considera solo técnicos activos y tickets cerrados en el mes.
                </p>
              </div>

              <button
                onClick={() => fetchWorstTecnicosTickets()}
                className={`inline-flex items-center gap-2 rounded-lg border border-orange-200 text-orange-700 px-3 py-1.5 text-xs hover:bg-orange-50 disabled:opacity-50 disabled:cursor-not-allowed ${loadingWorstTecnicosTickets ? "animate-spin" : ""
                  }`}
                disabled={loadingWorstTecnicosTickets}
              >
                {loadingWorstTecnicosTickets ? (
                  <LoadingOutlined />
                ) : (
                  <ReloadOutlined />
                )}{" "}
            ...
            ) : errorWorstTecnicosTickets ? (
              <div className="py-6 text-red-600">
                {errorWorstTecnicosTickets}
              </div>
            ) : worstTecnicosTickets.length === 0 ? (
              <div className="py-6 text-neutral-400">
                No hay técnicos activos disponibles.
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                {worstTecnicosTickets.map((t, index) => (
                  <div
                    key={t.tecnicoId}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <div className="rounded-full bg-orange-100 text-orange-800 text-xs font-semibold px-3 py-1">
                        {formatNumber(t.closedTickets)} cerrados
                      </div>
                    </div>
                    ...
                    <div className="mt-3 border-t border-orange-100 pt-3 text-xs text-slate-500">
                      {t.closedTickets === 0
                        ? "Sin tickets cerrados en el período."
                        : "Bajo volumen de tickets cerrados en el período."}
                    </div>
                  </div>
                ))}
              </div>
