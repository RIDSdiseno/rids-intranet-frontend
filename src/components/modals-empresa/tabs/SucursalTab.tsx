// ./../modals-empresa/tabs/SucursalTab.tsx
...
const SucursalesTab: React.FC<Props> = ({ empresaId, canEdit = true }) => {
    ...
    const load = async () => {
        try {
            setLoading(true);
            const { data: json } = await http.get(
                `/ficha-empresa/${empresaId}/sucursales`
            );
    ...
    useEffect(() => {
        ...
        if (editingSucursalId) {
            http
                .get(`/ficha-empresa/sucursales/${editingSucursalId}`)
                .then(({ data }) => {
                    form.setFieldsValue({
                        nombre: data.nombre ?? "",
            ...
            http
                .get(`/ficha-empresa/sucursales/${editingSucursalId}/red`)
                .then(({ data: red }) => {
    ...
    }, [modalOpen, editingSucursalId, form, canEdit]);
    ...
    const openEdit = (sucursalId: number) => {
        if (!canEdit) return;
    ...
    const openDelete = (sucursalId: number) => {
        if (!canEdit) return;

        setDeleteSucursalId(sucursalId);
    };

    const onSave = async () => {
        if (!canEdit) {
            message.warning("No tienes permisos para editar sucursales");
            return;
        }
        ...
        try {
            ...
            if (editingSucursalId) {
                await http.put(
                    `/ficha-empresa/sucursales/${editingSucursalId}`,
                    sucursalData
                );
                sucursalId = editingSucursalId;
            } else {
                const { data: saved } = await http.post(
                    `/ficha-empresa/${empresaId}/sucursales`,
                    sucursalData
                );
            ...
            if (hasWifiData) {
                await http.put(
                    `/ficha-empresa/sucursales/${sucursalId}/red`,
                    redSucursal
                );
            }
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">
                        {data.length} sucursal{data.length !== 1 ? "es" : ""} registrada
                        {data.length !== 1 ? "s" : ""}
                    </p>
                </div>
                                        ...
                                        <Space>
                                            ...
                                            <Tooltip title="Eliminar sucursal">
                                                <Button
                                                    ...
                                                    danger
                                                    icon={<DeleteOutlined />}
                                                    onClick={() => openDelete(sucursal.id_sucursal)}
                                                />
                                            </Tooltip>
                                        </Space>
                                    ) : null
