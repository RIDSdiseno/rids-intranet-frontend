...
export type ContentZoom = "80" | "90" | "100" | "110" | "120";
...
function normalizeContentZoom(value: unknown): ContentZoom {
    if (
        value === "80" ||
        value === "90" ||
        value === "100" ||
    ...
    return DEFAULT_SETTINGS.contentZoom;
}
...
export function readInitialSettings(): AccessibilitySettings {
    try {
        ...
        return {
            ...
            contentZoom: normalizeContentZoom(p.contentZoom),
        };
    } catch {
...
export function applyAccessibilityClasses(s: AccessibilitySettings): void {
    ...
    root.classList.add(
        `a11y-theme-${theme}`,
        `a11y-font-${s.fontSize}`,
        `a11y-contrast-${s.contrast}`,
        `a11y-motion-${s.motion}`
    );
    ...
    if (s.readableSpacing) {
        root.classList.add("a11y-readable-spacing");
    }

    const zoom = Number(s.contentZoom) / 100;
    const width = `${100 / zoom}%`;

    root.style.setProperty("--app-content-zoom", String(zoom));
    root.style.setProperty("--app-content-zoom-width", width);
}
